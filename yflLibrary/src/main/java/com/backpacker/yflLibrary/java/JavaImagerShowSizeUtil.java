...
public class JavaImagerShowSizeUtil {
    ...
    public static void setImgerSizeShow(ImageView img, int WithSize, boolean isWith, int with, int height,boolean isMul) {
        ...
        try {
            ...
            //宽高比
            if (isWith) {
                size = JavaArithUtil.div(with, height, 2);
            } else {
                //高宽比
                size = JavaArithUtil.div(height, with, 2);

            }
            if (isMul)
                div = JavaArithUtil.mul(WithSize, size, 2);
            else
                div = JavaArithUtil.div(WithSize, size, 2);
    ...
    public static void setImgerSizeShow(ImageView img, int height, boolean isWith, double size) {
        ...
        try {
            double div = 0.0;
            if (isWith)
                div = JavaArithUtil.div(height, size, 2);
            else
                div = JavaArithUtil.mul(height, size, 2);
            ViewGroup.LayoutParams layoutParams = img.getLayoutParams();
            layoutParams.height = (int) div;
            img.setLayoutParams(layoutParams);
        } catch (IllegalAccessException e) {
    ...
    public static void setImgerSizeShow(Context mContent, ImageView img, double size) {
        ...
        try {
            double div = JavaArithUtil.div(widthPixels, size, 2);
            ViewGroup.LayoutParams layoutParams = img.getLayoutParams();
            layoutParams.height = (int) div;
            img.setLayoutParams(layoutParams);
        } catch (IllegalAccessException e) {
    ...
    public static void setImgerSizeShow(Context mContent, ImageView img, String size) {
        ...
        try {
            double sizeDoubul = JavaArithUtil.div(Double.valueOf(split[0]), Double.valueOf(split[1]), 2);
            WindowManager wm = (WindowManager) mContent.getSystemService(Context.WINDOW_SERVICE);
            DisplayMetrics dm = new DisplayMetrics();
            wm.getDefaultDisplay().getMetrics(dm);
            //屏幕宽度
            int widthPixels = dm.widthPixels;
            try {
                double div = JavaArithUtil.div(widthPixels, sizeDoubul, 2);
                ViewGroup.LayoutParams layoutParams = img.getLayoutParams();
                layoutParams.height = (int) div;
                img.setLayoutParams(layoutParams);
            } catch (IllegalAccessException e) {
