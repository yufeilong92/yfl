...
import com.backpacker.yflLibrary.kotlin.KotlinT;
...
//            }
    ...
    public static void baiduGuide(Context context, String appName,double[] location) {
        double[] baiduLoc = JavaGpsUtil.gcj02_To_Bd09(location[0], location[1]);
...
//intent = Intent.getIntent("intent://map/direction?origin=latlng:34.264642646862,108.95108518068|name:我家&destination=大雁塔&mode=driving®ion=西安&src=yourCompanyName|yourAppName#Intent;scheme=bdapp;package=com.baidu.BaiduMap;end");
    ...
    public static void selectMapDialog(Context mContext, Double lat, Double lng,  Integer type,String appName) {
        ...
        {
            case 1:
                if (!JavaOpenOtherMapUtil.isAvilible(mContext, JavaOpenOtherMapUtil.BAIDU_PACKAGENAME)) {
                    KotlinT.INSTANCE.showToast(mContext, "请安装百度地图");
                }
                double[] array={lat,lng};
                JavaOpenOtherMapUtil.baiduGuide(mContext, appName,array);
                break;
            case 2:
                if (!JavaOpenOtherMapUtil.isAvilible(mContext, JavaOpenOtherMapUtil.GAODE_PACKAGENAME)) {
                    KotlinT.INSTANCE.showToast(mContext, "请安装高德地图");
                }
                double[] array1={lat,lng};
                JavaOpenOtherMapUtil.gaodeGuide(mContext,appName, array1);
                break;
            case 3:
                if (!JavaOpenOtherMapUtil.isAvilible(mContext, JavaOpenOtherMapUtil.TENCENT_PACKAGENAME)) {
                    KotlinT.INSTANCE.showToast(mContext, "请安装腾讯地图");
                }
                double[] array2={lat,lng};
                JavaOpenOtherMapUtil.tencentGuide(mContext,appName, array2);
                break;
            default:
        }
