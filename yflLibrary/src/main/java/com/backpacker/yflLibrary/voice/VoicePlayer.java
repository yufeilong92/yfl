...
public class VoicePlayer {
     ...
     * 有界面的语音播放
     */
    public synchronized void play(Context mContext, final VoiceTask task, final Runnable scheduleNext) {
        ...
        try {
            ...
            if (url == null&&TextUtils.isEmpty(task.getAssesName())) {
                if (listener != null) {
            ...
            if (TextUtils.isEmpty(task.getAssesName())&&!TextUtils.isEmpty(task.getUrl())) {
                mediaPlayer.setDataSource(url);
            } else if (!TextUtils.isEmpty(task.getAssesName())&&TextUtils.isEmpty(task.getUrl())){
                AssetFileDescriptor openFd = mContext.getAssets().openFd(task.getAssesName());
                mediaPlayer.setDataSource(openFd.getFileDescriptor(), openFd.getStartOffset(), openFd.getLength());
            }else {
                mediaPlayer.setDataSource(url);
            }
//            AssetFileDescriptor openFd = mContext.getAssets().openFd("rcc.mp3");
//            mediaPlayer.setDataSource(url);
//            mediaPlayer.setDataSource(openFd.getFileDescriptor(),openFd.getStartOffset(),openFd.getLength());
