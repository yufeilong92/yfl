...
public class VoiceManager {
     ...
     * 添加任务:推送时调用
     */
    public synchronized void submit(Context mContext,VoiceTask task) {
        try {
            taskQueue.add(task);
            if (!voicePlayer.isPlaying()) {
                takeOneToPlay(mContext);
            }
        } catch (Exception e) {
     ...
     * 立即播放:点击时调用
     */
    public synchronized void executeNow(Context mContext,final VoiceTask task) {
        currentTask = task;
        voicePlayer.play(mContext,task, new Runnable() {
            @Override
            public void run() {
                currentTask = null;
                if (!task.isCanceled()) {
     ...
     */
    public void startTask(Context mContext) {
        takeOneToPlay(mContext);
    }
     ...
     * 执行一个任务
     */
    private void takeOneToPlay(Context mContext) {
        VoiceTask task = taskQueue.poll();
        if (task != null) {
            // 有任务执行
            executeNow(mContext,task);
